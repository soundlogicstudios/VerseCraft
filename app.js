...
// Patch: HP Clamp Rule (canonical MAX_HP)
// - Add STATE.resources.MAX_HP as the single source of truth for Max HP
// - Clamp all HP changes to MAX_HP
// - Initialize MAX_HP from story defaults on new runs + normalize on loads
...
(function () {
  ...
  function normalizeState(s, storyObj) {
    ...
    if (s.resources.HP == null) s.resources.HP = (d.resources?.HP ?? 10);
    if (s.resources.REPUTATION == null) s.resources.REPUTATION = 0;

    // --- HP Clamp Rule: canonical MAX_HP ---
    // If MAX_HP missing, set it from story defaults first, else current HP.
    if (s.resources.MAX_HP == null) {
      const defaultMax = Number(d.resources?.HP ?? 10);
      const cur = Number(s.resources.HP ?? defaultMax);
      s.resources.MAX_HP = Number.isFinite(defaultMax) ? defaultMax : (Number.isFinite(cur) ? cur : 10);
    }
    // Clamp current HP to MAX_HP immediately
    s.resources.HP = clamp(Number(s.resources.HP ?? 0), 0, Number(s.resources.MAX_HP));
  ...
  // HP Clamp Rule: MAX_HP is canonical (fallbacks are defensive)
  function getMaxHP() {
    const fromState = Number(STATE?.resources?.MAX_HP);
    if (Number.isFinite(fromState) && fromState > 0) return fromState;

    const fromStory = Number(STORY?.save?.defaults?.resources?.HP ?? 10);
    if (Number.isFinite(fromStory) && fromStory > 0) return fromStory;
  ...
  function applyOp(op) {
    ...
    switch (op.op) {
      case "ADD_RESOURCE": {
        ...
        if (res === "HP") {
          // HP Clamp Rule applied through setHP (clamps to MAX_HP)
          setHP(getHP() + amt);
      ...
      case "SET_RESOURCE": {
        ...
        if (res === "HP") {
          ...
          STATE.resources.REPUTATION = val;
        } else if (res === "MAX_HP") {
          // Allow future systems to set max HP explicitly, but keep HP clamped
          const v = Math.max(1, Number(val ?? 10));
          STATE.resources.MAX_HP = v;
          setHP(getHP());
        }
        break;
      }
  ...
  function startNewRun() {
    ...
    // Ensure MAX_HP is set on brand-new runs
    if (STATE.resources.MAX_HP == null) {
      STATE.resources.MAX_HP = Number(STORY.save?.defaults?.resources?.HP ?? 10);
    }
    // Clamp current HP to MAX_HP
    setHP(STATE.resources.HP);
      ...
      card.onclick = async () => {
        ...
        $("#btnSetStory").onclick = async () => {
          try {
            ...
            // Ensure MAX_HP is set after switching stories (defensive)
            if (STATE.resources.MAX_HP == null) {
              STATE.resources.MAX_HP = Number(STORY.save?.defaults?.resources?.HP ?? 10);
            }
            setHP(STATE.resources.HP);
  ...
  async function boot() {
    ...
    // Ensure MAX_HP is always set, then clamp HP.
    if (STATE.resources.MAX_HP == null) {
      STATE.resources.MAX_HP = Number(STORY.save?.defaults?.resources?.HP ?? 10);
    }
    setHP(STATE.resources.HP);

    // Avoid starting dead due to corrupted saves
    if (getHP() <= 0) setHP(1);
