// app.js — VerseCraft demo (Story Library + Tabs + StoryLogic item use)
// Root GitHub Pages. Logo stored in /assets.
// Full-file replace.
...
// Logo in assets folder (confirmed)
...
(function () {
  ...
  function normalizeState(s, storyObj) {
    ...
    // Ensure baseline loadout exists (for demo consistency)
    ...
    app.innerHTML = `
      <div class="panel menu-hero menu-center splash" id="tapStart">
        <div class="menu-brand">
          ...
          <h2 class="menu-title">VERSECRAFT</h2>
          <div class="menu-subtitle">${esc(STORY_META?.title || "Lorecraft Tutorial")} · ${esc(STORY_META?.subtitle || "")}</div>
        </div>
        <div class="tap-hint"><span class="tap-pill">Tap to Start</span></div>
        <div class="menu-footnote small">Local saves · Root GitHub Pages · Demo build</div>
      </div>
    `;
      ...
      <div class="panel menu-hero menu-center">
        <div class="menu-brand">
          ...
          <div class="menu-subtitle">
            Current Story: <b>${esc(STORY_META?.title || "Lorecraft Tutorial")}</b> — ${esc(STORY_META?.subtitle || "")}
          </div>
        </div>
        ...
        <div class="menu-footnote">
          Tip: Continue shows saves for the currently selected story.
        </div>
      </div>
    `;
  ...
  function startNewRun() {
    ...
    STATE.isDead = false;
    // Keep baseline loadout for demo; story defaults can override later.
  ...
  async function boot() {
    if (!app) return;
    app.innerHTML = `<div class="panel"><p>Loading VerseCraft…</p></div>`;
